...
function starting() {
  if (window.innerWidth > 1299) {
    ...
    alert(
      "your creen is too small to play this game, you need at least 1300 pixels and your screen is " +
        window.innerWidth +
        " pixels"
    );
  }
...
//   playerArr = JSON.parse(localStorage.allPlayers); // récupère le array des joueurs
  ...
  if (playerArr.length < 5 || newPlayer.hisPoints >  playerArr[4].hisPoints) {
...
function affiche5Winners() {
  ...
  if (playerArr.length > 0) {
    ...
    <div class="date-winner"><p>${x.hisDate}</p></div>
